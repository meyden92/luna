...
export default function ModelManager({ models }: ModelManagerProps) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [modelToDelete, setModelToDelete] = useState<GenerationModel | null>(null);

  const { mutate: deleteModel, isPending: isDeleting } = useAppMutation(deleteGenerationModel, {
    successMessage: 'Model deleted successfully',
    errorMessage: 'Failed to delete model',
    invalidates: [queryKeys.adminModels.generation],
    onSuccess: () => {
  ...
  } = useAppMutation(setGenerationModelActive, {
    successMessage: (data) => `${data.model.label} ${data.model.isActive ? 'activated' : 'deactivated'}`,
    errorMessage: 'Failed to update model status',
    invalidates: [queryKeys.adminModels.generation],
    onSuccess: ({ model }) => {
      queryClient.setQueryData<GenerationModel[]>(queryKeys.adminModels.generation, (current) =>
        current?.map((item) => (item.id === model.id ? model : item)),
      );
      queryClient.setQueryData(queryKeys.adminModels.generationById(model.id), model);
    },
  });

  const handleDelete = async (model: GenerationModel) => {
