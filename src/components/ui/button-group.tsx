...
function ButtonGroupText({ className, render, ...props }: useRender.ComponentProps<'div'>) {
  return useRender({
    ...
    props: mergeProps<'div'>(
      {
        className: cn(
          "bg-muted/70 gap-2 rounded-lg border border-border/70 px-2.5 text-sm font-medium shadow-sm [&_svg:not([class*='size-'])]:size-4 flex items-center [&_svg]:pointer-events-none",
          className,
        ),
      },
      props,
