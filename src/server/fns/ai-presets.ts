...
/**
 * A saved set of Create settings for one generation model. `fieldValues` holds
 * the model's own field values (shape, count, steps, seed written onto the
 * fields the model declares), so applying one reads them back the same way a
 * finished run is read.
 */
